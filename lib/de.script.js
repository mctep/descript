...
de.Script = function(config, modules) {
    ...
    //  Кэш результатов выполнений блоков (с key/maxage).
    this._results = {};

    //  Кэш страниц.
    var pages = this._pages = {};
    this.on('page-changed', function(e, filename) {
        pages[filename] = null;
    });

    //  Кэш инклюдов.
    var includes = this._includes = {};
    this.on('include-changed', function(e, filename) {
        includes[filename] = null;
    });

    //  Кэш файлов.
    var files = this._files = {};
    this.on('file-changed', function(e, filename) {
        files[filename] = null;
    });
...
no.extend(de.Script.prototype, no.Events);
...
de.Script.prototype.start = function() {
    ...
    if (cluster_.isMaster) {
        ...
        var server = this.servier = http_.createServer(function (req, res) {
            ...
            /// path = path_.resolve(config.rootdir, path);

            var block = pages[path];
            if (!block) {
                block = pages[path] = new de.Block.Include(path, that);

                no.de.file.watch('page-changed', path);
            }
